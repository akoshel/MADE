...
class Asset(ABC):
    ...
    def calculate_revenue(self, years: int, forecast_strategy=None) -> float:
        ...
        revenue = forecast_strategy.fixup_reveue_prediction(revenue)
...
class ForecastStrategy(ABC):
    @abstractmethod
    def fixup_reveue_prediction(self, revenue):
        raise NotImplementedError
...
class Bank:
    ...
    def calculate_revenue(self, year):
        ...
        for asset_name, asset in self._asset_collection.items():
            ...
            #asset_revenue = self._forecast_strategy.fixup_reveue_prediction(asset_revenue)
    ...
    def print_report(self, years):
        ...
        for year in years:
            excpected_revenue = self.calculate_revenue(year)
            print(f"{year:5} {excpected_revenue:10.3f}")
