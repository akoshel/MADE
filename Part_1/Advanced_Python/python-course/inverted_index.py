...
class InvertedIndex:
    ...
    def query(self, words: List[str]) -> Set[str]:
        ...
        try:
            ...
            logger.debug(' %s missing in inverted index', words[0])
            docs_id = set()
        for word in words:
            try:
                ...
                docs_id = set()
                logger.debug("%s missing in inverted index", word)
        return docs_id
...
def load_documents(filepath: str) -> Dict[str, List[int]]:
    ...
    logger.info(f'loading from %s', filepath)
...
def callback_build(arguments):
    logger.debug('build inverted index from %s and dump to %s', arguments.dataset, arguments.output)
...
def callback_query(arguments):
    logger.info('Load from %s', arguments.index)
