...
class InvertedIndex:
    ...
    def query(self, words: List[str]) -> Set[str]:
        ...
        try:
            ...
            print(f'{words[0]} missing in inverted index', file=sys.stderr)
            docs_id = set()
        for word in words:
            try:
                ...
                docs_id = set()
                print(f'{word} missing in inverted index', file=sys.stderr)
        return docs_id

    def dump(self, filepath: str):
        """Extract inverted index dictionary to json"""
        print(f'Dumping {filepath}', file=sys.stderr)
...
def callback_build(arguments):
    print(f'build inverted index from {arguments.dataset} and dump to {arguments.output}', file=sys.stderr)
...
def callback_query(arguments):
    print(f'Load from {arguments.index}', file=sys.stderr)
