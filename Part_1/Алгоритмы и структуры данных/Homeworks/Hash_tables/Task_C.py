...
class Map:
    ...
    def put(self, key, value):
        ...
        if self.table[hash_key].head == None:
            ...
            node.map_prev.map_next = node.map_next
            node.map_next.map_prev = node.map_prev
            node = add_node
            return
        else:
            node = self.table[hash_key].head
...
def main() -> None:
    my_map = Map()
    stdin = fileinput.input('t2.txt')
    sstdin, stdout = IOWrapper(sys.stdin), IOWrapper(sys.stdout)

    for inp in stdin:
        #print([t for t in my_map.table], inp)
        inp = inp.split(' ')
