...
def get_min_powers(n):
    min_power = [0]
    for i in range(1, n + 1):
        min_power.append(min_power[i - 1])
        if 1 << min_power[i] <= i:
            min_power[i] += 1
    return min_power
...
def get_min(l: int, r: int, min_matrix: list, arr: list, min_power: list):
    if l == r:
        return arr[l]
    j = min_power[r - l] - 1
...
min_power = get_min_powers(n)
arr = get_a(a1, n)
min_matrix = get_sparse_table(arr, n)

for i in range(1, m + 1):
    res = get_min(min(u, v) - 1, max(u, v) - 1, min_matrix, arr, min_power)
    if i < m:
        u = get_u(u, n, res, i)
        v = get_v(v, n, res, i)
    else:
